...
from tensorflow_core.python.keras.layers import Embedding, GlobalMaxPooling1D, Dense, Dropout, BatchNormalization, \
    GlobalAveragePooling1D, concatenate, Activation, Conv1D, MaxPooling1D, AveragePooling1D
...
def construct_keras_api_model():
    ...
    embedded_creative_id = Embedding(
        creative_id_window, embedding_size, name='creative_id_embedded', weights=[embedding_weights], trainable=False
    )(input_creative_id)
    ...
    Conv_GM_creative_id = Conv1D(
        embedding_size, 5, 3, activation='relu', name='Conv_GM_Conv_0101'
    )(embedded_creative_id)
    Conv_GM_x = MaxPooling1D(8, name='Conv_GM_Conv_MaxPool_0101')(Conv_GM_creative_id)
    ...
    Conv_GA_creative_id = Conv1D(
        embedding_size, 5, 3, activation='relu', name='Conv_GA_Conv_0101'
    )(embedded_creative_id)
    Conv_GA_x = AveragePooling1D(8, name='Conv_GA_Conv_AveragePool_0101')(Conv_GA_creative_id)
...
history = model.fit(
    X_train, y_train, epochs=epochs, batch_size=batch_size, validation_split=0.2, use_multiprocessing=True, verbose=2
)
