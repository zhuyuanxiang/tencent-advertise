...
def output_result(results, predictions):
    ...
    if label_name == 'age':
        ...
        print("sum(abs(predictions>0.5-y_test_scaled))/sum(y_test_scaled) = error% =", sum(abs(predict_gender - y_test)) / sum(y_test) * 100, '%')
...
word2vec = KeyedVectors.load("save_model/word2vec/vectors_{0}_{1}_{2}.kv".format(embedding_size, embedding_window, creative_id_window))
...
def construct_keras_api_model():
    ...
    #     creative_id_window,embedding_size,weights=[embedding_weights],name='Embedded_no_time_no_repeat',trainable=False
    # )(input_no_time_no_repeat)
    # ==================================================================================
    input_fix_creative_id = Input(shape=(math.ceil(time_id_max / period_days) * period_length), dtype='int32', name='Input_fix_creative_id')
    embedded_fix_creative_id = Embedding(
        creative_id_window, embedding_size, weights=[embedding_weights], name='Embedded_fix_creative_id', trainable=False
    ...
    #     creative_id_window,embedding_size,weights=[embedding_weights],name='Embedded_no_time_with_repeat',trainable=False
    # )(input_no_time_with_repeat)

    # ----------------------------------------------------------------------
    GM_creative_id = GlobalMaxPooling1D(name='GM_creative_id')(embedded_fix_creative_id)
    ...
    # ----------------------------------------------------------------------
    GA_creative_id = GlobalAveragePooling1D(name='GA_creative_id')(embedded_fix_creative_id)
    ...
    # ----------------------------------------------------------------------
    Conv_creative_id = Conv1D(embedding_size, 15, 5, activation='relu', name='Conv_creative_id')(embedded_fix_creative_id)
    Conv_GM_x = MaxPooling1D(7, name='Conv_GM_Conv_MaxPool_0101')(Conv_creative_id)
    Conv_GM_x = Conv1D(embedding_size, 2, 1, activation='relu', name='Conv_GM_Conv_0101')(Conv_GM_x)
    ...
    # ----------------------------------------------------------------------
    Conv_GA_x = AveragePooling1D(7, name='Conv_GA_Conv_AveragePool_0101')(Conv_creative_id)
    Conv_GA_x = Conv1D(embedding_size, 2, 1, activation='relu', name='Conv_GA_Conv_0101')(Conv_GA_x)
    ...
    # ----------------------------------------------------------------------
    LSTM_Conv_creative_id = Conv1D(embedding_size, 14, 7, activation='relu', name='LSTM_Conv_creative_id')(Conv_creative_id)
    LSTM_creative_id = LSTM(embedding_size, return_sequences=True, name='LSTM_creative_id_0101')(LSTM_Conv_creative_id)
    ...
    print('-' * 5 + ' ' * 3 + "编译模型" + ' ' * 3 + '-' * 5)
    keras_api_model.compile(optimizer=optimizers.RMSprop(lr=RMSProp_lr), loss=losses.binary_crossentropy, metrics=[metrics.binary_accuracy])
...
#     padding='pre'
# )
X_train_fix_creative_id = np.load('save_data/fix_{0}_{1}_{2}/x_train_fix_{3}.npy'.format(period_days, period_length, creative_id_window, label_name),
                                  allow_pickle=True)[:, 0]
...
#     padding='pre'
# )
X_test_fix_creative_id = np.load('save_data/fix_{0}_{1}_{2}/x_test_fix_{3}.npy'.format(period_days, period_length, creative_id_window, label_name),
                                 allow_pickle=True)[:, 0]
