/* 
    统计 `click_log_500k` 中的 `creative_id` 素材
    sum_creative_click_times：每个素材出现的次数 
    sum_user_id_times：每个素材的访问用户数
 */
DROP TABLE `number_creative_id_500k`;

CREATE TABLE `number_creative_id_500k` (
...
/* 
    统计 `click_log_500k` 中的 `user_id` 的统计数据
    sum_user_click_times : 每个用户访问素材的次数 
    sum_creative_id_times : 每个用户访问素材的种类
 */
DROP TABLE `number_user_id_500k`;

CREATE TABLE `number_user_id_500k` (
...
/* 
    分别计算 `click_log_500k`中每个`creative_id`的 `tf`,`idf`,`tf-idf`的值
    tf : 素材出现的次数越多越重要，单个素材出现次数/所有素材出现次数
    idf: 访问素材的用户越少越重要，所有用户的数目/访问单个素材的用户数目
 */
DROP TABLE `tf_idf_500k`;

CREATE TABLE `tf_idf_500k` (
    `creative_id` INT NOT NULL,
    `tf_value` FLOAT NULL,
...
/* 插入 tf 值 */
INSERT INTO
    `tf_idf_500k` (`creative_id`, `tf_value`)
SELECT
    A.creative_id,
    (A.sum_creative_click_times / 1075963) AS tf_value
FROM
    number_creative_id_500k AS A;
...
SELECT
    A.`user_id`
FROM
    `number_user_id_500k` AS A
ORDER BY
    sum_user_click_times DESC;

/* 创建训练数据，沿用 `click_log_500k` 中的数据，重新编码 `creative_id` 和 `user_id`  */
