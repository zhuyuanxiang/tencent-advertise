...
def export_generate_data(lst_data):
    ...
    show_title("加工数据为 91 天的序列数据，每天为3个特征(最大值、最小值、平均值)96维数据")
    x_data = generate_day_sequence_data(lst_data)
    save_model_data(x_data, config.data_file_path + 'x_data', '中间数据集')
...
def export_day_list_data():
    ...
    save_model_data(lst_data, config.data_file_path + 'lst_data', '中间数据集')
    save_model_data(y_data, config.data_file_path + 'y_data', '中间数据集')
