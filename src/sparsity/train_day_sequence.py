...
def construct_model():
    input_creative_id = Input(shape=(time_id_max, 3 * embedding_size), name='creative_id')
    model_input = [input_creative_id]
    x_output = build_residual(model_input[0], embedding_size * 6, 3)
    x_output = MaxPooling1D(7)(x_output)
    x_output = LSTM(embedding_size * 8, dropout=0.2, recurrent_dropout=0.2)(x_output)
...
def main():
    ...
    x_train_val.reshape([user_id_max, time_id_max, 3 * embedding_size])
