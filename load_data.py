...
import numpy as np
import pandas as pd
from show_data import show_example_data, show_word2vec_data, show_original_x_data, show_original_data
...
def load_original_data():
    ...
    x_csv[:, 3] = x_csv[:, 3] + (1 - 1)  # click_times 的数据已经是值，不是索引，不需要减 1
    # --------------------------------------------------
...
def load_data(file_name, data_type='原始数据集'):
    print("加载{0}：{1}-->".format(data_type, file_name), end='')
    data = np.load(file_name + '.npy', allow_pickle=True)
    print(data_type + ':{}-->加载成功！'.format(len(data)))
